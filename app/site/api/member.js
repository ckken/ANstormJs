...
o.login = function (req, res) {
    ...
    if (name != '' && !_S.Guser.login) {
        ...
        D('Member').model().findOne(where, function (err, data) {
            ...
            if (data != null) {
                ...
                var d = {};
                d.logip = req.ip;
                D('Member').update({_id: data._id}, d, function (err, row) {
                });
...
o.register = function (req, res) {
    ...
    if (d.name != '' || d.email != '') {
        D('Member').model().count({email: d.email}, function (err, count) {
            if (!count) {
                D('Member').model().count({name: d.name}, function (err, count) {
                    var data = d;
                    if (!count) {
